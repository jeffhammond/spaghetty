...
def generate_tester(ofile, transpose_order, reps):
    A = transpose_order[0]
    B = transpose_order[1]
    C = transpose_order[2]
    D = transpose_order[3]
    ...
    ofile.write('     &                dim1, dim2, dim3, dim4, rank)\n')
    ofile.write('        implicit none\n')
    ofile.write('! external variables\n')
    ofile.write('        integer dim1, dim2, dim3, dim4, dim1234, rank\n')
    ofile.write('        double precision sorted(dim1*dim2*dim3*dim4)\n')
    ofile.write('        double precision unsorted(dim1*dim2*dim3*dim4)\n')
    ofile.write('        double precision reference(dim1*dim2*dim3*dim4)\n')
    ofile.write('! internal variables\n')
    ofile.write('        integer errors\n')
    ofile.write('        integer loop, omp, fac, lang, r\n')
    ofile.write('        integer loops(4,24) \n')
    ofile.write('        integer bestloop(4,5,2) ! best loop order for (fac,omp,lang) \n')
    ofile.write('        integer bestlang(5,2)   ! best lang for (fac,omp) \n')
    ...
    ofile.write('        double precision dt(24,5,2,2) ! time for (loop,fac,omp,lang) \n')
    ofile.write('        double precision besttime(5,2) ! best time for (fac,omp,lang) \n')
    ofile.write('        double precision wtime\n')
    ofile.write('        external wtime\n')
    ofile.write('        character*20 labels(5,2,2)\n')
    ofile.write('        dim1234 = dim1*dim2*dim3*dim4\n')
    ofile.write('        do lang = 1, 2\n')
    ofile.write('         do omp  = 1, 2\n')
    ofile.write('          do fac  = 1, 5\n')
    ofile.write('           labels(fac,omp,lang) = \'UNDEFINED\' \n')
    ofile.write('           do loop = 1, 24\n')
    ofile.write('            dt(loop,fac,omp,lang) = 1000000.0\n')
    ofile.write('           enddo\n')
    ofile.write('          enddo\n')
    ofile.write('         enddo\n')
    ofile.write('        enddo\n')
        ...
        for OpenMP in [False,True]:
            for Language in ['f','c']:
                ...
                for loop_order in generate_permutation_list(Debug):
                    ...
                    ofile.write('        labels('+str(fac)+','+str(omp)+','+str(lang)+') = \''+variant+'('+str(Factor)+','+str(Accumulate)+')\' \n')
                    ofile.write('!******** '+str(a)+','+str(b)+','+str(c)+','+str(d)+' ********\n')
                    ofile.write('        fac  = '+str(fac)+' \n')
                    ofile.write('        lang = '+str(lang)+' \n')
                    ...
                    ofile.write('          dt(loop,fac,omp,lang) = (t1-t0)/'+str(reps)+'\n')
                    ofile.write('        else\n')
                    ofile.write('          dt(loop,fac,omp,lang) = 10000000.0\n')
    ...
    ofile.write('        write(6,2000) rank, '+str(A)+','+str(B)+','+str(C)+','+str(D)+'\n')
    ofile.write('        write(6,1500) rank,\'old_sort_4   \',dt0,(8*dim1234)/dt0\n')
    ofile.write('        write(6,1500) rank,\'old_sortacc_4\',dt1,(8*dim1234)/dt1\n')
    ofile.write('        do omp  = 1, 2\n')
    ofile.write('         do fac  = 1, 5\n')
    ...
    ofile.write('           do loop = 1, 24\n')
    ofile.write('            do lang = 1, 2\n')
    ofile.write('             if (dt(loop,fac,omp,lang).lt.besttime(fac,omp)) then\n')
    ofile.write('               besttime(fac,omp)   = dt(loop,fac,omp,lang)\n')
    ofile.write('               bestlang(fac,omp)   = lang\n')
    ...
    ofile.write('             lang = bestlang(fac,omp) \n')
    ofile.write('             write(6,1000) rank,\'best \',labels(fac,omp,lang),\n')
    ...
    ofile.write('     &       (8*dim1234)/besttime(fac,omp)\n')
    ofile.write('             do loop = 1, 24\n')
    ofile.write('              do lang = 1, 2\n')
    ofile.write('               if ((dt(loop,fac,omp,lang).lt.\n')
    ofile.write('     &                 1.2*besttime(fac,omp)) .and.\n')
    ofile.write('     &             (dt(loop,fac,omp,lang).gt.\n')
    ofile.write('     &                 besttime(fac,omp))) then\n')
    ofile.write('                 write(6,1000) rank,\'good \',labels(fac,omp,lang),\n')
    ofile.write('     &           loops(1,loop),loops(2,loop),            \n')
    ofile.write('     &           loops(3,loop),loops(4,loop),            \n')
    ofile.write('     &           dt(loop,fac,omp,lang),dtX/dt(loop,fac,omp,lang),  \n')
    ofile.write('     &           (8*dim1234)/dt(loop,fac,omp,lang)\n')
    ofile.write('               endif\n')
    ofile.write('              enddo\n')
    ofile.write('             enddo\n')
    ofile.write('           endif\n')
...
def generate_test_driver(Debug, Compiler, subdir, underscoring, rev, flags, trans_list):
    ...
    cfile.write('{\n')
    cfile.write('    int rank=0, size=1;\n')
    cfile.write('#ifdef USE_MPI\n')
    cfile.write('    int provided;\n')
    cfile.write('    MPI_Init_thread(&argc,&argv,MPI_THREAD_FUNNELED,&provided);\n')
    cfile.write('    if (provided<MPI_THREAD_FUNNELED) \n')
    cfile.write('       MPI_Abort(MPI_COMM_WORLD, provided);\n')
    ...
    cfile.write('    int dim1234 = dim1*dim2*dim3*dim4;\n\n')
    cfile.write('#ifdef _OPENMP\n')
    cfile.write('    int nthreads = omp_get_max_threads();\n')
    cfile.write('#else\n')
    cfile.write('    int nthreads = 1;\n')
    cfile.write('#endif\n\n')
    cfile.write('    if (rank==0) {\n')
    cfile.write('      printf(\"SPAGHETTY: generator2.py r'+str(rev)+' - %d MPI ranks\\n\", size);\n')
    ...
    cfile.write('    rc = posix_memalign((void**)&unsorted,  4096, dim1234*sizeof(double)); assert(rc==0 && unsorted!=NULL);\n')
    cfile.write('    rc = posix_memalign((void**)&sorted,    4096, dim1234*sizeof(double)); assert(rc==0 && sorted!=NULL);\n')
    cfile.write('    rc = posix_memalign((void**)&reference, 4096, dim1234*sizeof(double)); assert(rc==0 && reference!=NULL);\n\n')
    ...
    for transpose_order in trans_list:
        ...
        cfile.write('      printf(\"rank %d is running test_'+test_name+'\\n\", rank);\n')
        cfile.write('      fflush(stdout);\n')
        cfile.write('      test_'+test_name+'(reference, unsorted, sorted, &dim1, &dim2, &dim3, &dim4, &rank);\n')
        cfile.write('    }\n')
        hfile.write('void test_'+test_name+'(double * unsorted, double * sorted, double * reference,\n')
        hfile.write('                     int * dim1, int * dim2, int * dim3, int * dim4, int * rank);\n')
        i = i + 1
    cfile.write('    /* end testing */\n')
    cfile.write('    free(reference);\n')
    cfile.write('    free(sorted);\n')
    cfile.write('    free(unsorted);\n\n')
    cfile.write('    if (rank==0) printf("THE END\\n");\n')
    cfile.write('#ifdef USE_MPI\n')
...
def generate_all_subroutines(Debug, NewTester, Compiler, subdir, underscoring, trans_list):
    ...
    for transpose_order in trans_list: #generate_permutation_list(Debug):
        if NewTester:
            ...
            generate_tester(source_file, transpose_order, reps)
            source_file.close()
        for Language in ['f','c']:
