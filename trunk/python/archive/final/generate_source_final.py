...
for OpenMP in [True,False]:
    ...
    for Factor in [1.0,-1.0,0.5,-0.5]:
        if (Factor==1.0):
            variant = omp_name+'_copy'    
        elif (Factor==-1.0):
            variant = omp_name+'_neg'    
        elif (Factor==0.5):
            variant = omp_name+'_half'    
        elif (Factor==-0.5):
            variant = omp_name+'_nhalf'    
        ...
        for transpose_order in transpose_list:
            ...
            for loop_order in loop_list:
                ...
                c = loop_order[2]
                d = loop_order[3]
                subroutine_name = 'trans_'+A+B+C+D+'_loop_'+a+b+c+d+'_'+variant
                ...
                if OpenMP:
                    ...
                    source_file.write('!$omp& firstprivate(factor,dim1,dim2,dim3,dim4)\n')
                ...
                if (Factor==1.0):
                    ...
                    source_file.write('            sorted(j'+D+'+dim'+D+'*(j'+C+'-1+dim'+C+'*(j'+B+'-1+dim'+B+'*(j'+A+'-1)))) = \n')
                    source_file.write('     &    0.5*unsorted(j4+dim4*(j3-1+dim3*(j2-1+dim2*(j1-1))))\n')
                elif (Factor==-0.5):
                    source_file.write('            sorted(j'+D+'+dim'+D+'*(j'+C+'-1+dim'+C+'*(j'+B+'-1+dim'+B+'*(j'+A+'-1)))) = \n')
                    source_file.write('     &    -0.5*unsorted(j4+dim4*(j3-1+dim3*(j2-1+dim2*(j1-1))))\n')
