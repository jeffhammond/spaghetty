...
def generate_subroutine(inA,inB,inC,inD,outA,outB,outC,outD,blkA,blkB,blkC,blkD):
	SinA = str(inA)
	SinB = str(inB)
	SinC = str(inC)
	SinD = str(inD)
	...
	SblkC = str(blkC)
	SblkD = str(blkD)
	subroutine_name = 'transpose_in'+SinA+SinB+SinC+SinD+'_out'+SoutA+SoutB+SoutC+SoutD+'_blk'+SblkA+SblkB+SblkC+SblkD
	...
	for bA in range(0,blkA):
		for bB in range(0,blkB):
			for bC in range(0,blkC):
				for bD in range(0,blkD):
