...
print fortran_compiler+' '+fortran_opt_flags+' -c tce_sort_hirata_openmp_noflop.F'
os.system(fortran_compiler+' '+fortran_opt_flags+' -c tce_sort_hirata_openmp_noflop.F')
os.system('ar -r '+lib_name+' tce_sort_hirata_openmp_noflop.o')
...
for transpose_order in transpose_list:
    ...
    C = transpose_order[2]
    D = transpose_order[3]
    driver_name = 'transpose_'+A+B+C+D+'superflush_omp'
    print driver_name
    source_name = driver_name+'_driver.F'
    lst_name = driver_name+'_driver.lst'
    source_file = open(source_name,'w')
    source_file.write('        PROGRAM ARRAYTEST2\n')
    if ( mpi ):
